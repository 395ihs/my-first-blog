...
def post_list(request):
    ...
    rpost = Log.objects.all().filter( created_date__lte=( datetime.now() - timedelta(minutes=1) ) )
    rpost_size = float(len(rpost))
    if (rpost_size == 0.0):
        rpost_size = 1.0
    temperature = sum([float(post.temperature) for post in rpost]) / rpost_size
    humidity = sum([float(post.humidity) for post in rpost]) / rpost_size
    dust = sum([float(post.dust) for post in rpost]) / rpost_size
    gas = sum([float(post.gas) for post in rpost]) / rpost_size
